...
export class UserController {
  ...
  static async AddBitcoin(ID: number, money: number) {
    const user = await UserRepo.findOneBy({ ID });
    if (!user) return false;
  ...
  static async GiveBitcoin(fromID: number, toID: number, money: number) {
    const remove = await this.RemoveBitcoin(fromID, money);
    if (!remove) return { success: false, errcode: "no_bitcoin" };
    const add = await this.AddBitcoin(toID, money);
    if (!add) return { success: false, errcode: "no_user" };

    return { success: true, errcode: "" };
  }

  static async RemoveBitcoin(ID: number, money: number, ignore: boolean = false) {
    const user = await UserRepo.findOneBy({ ID });
    if (!user) return null;
